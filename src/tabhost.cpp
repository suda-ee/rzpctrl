...
void tabhost::getDataRate()
{
    ...
    if (dataSocket->write(sendingMsg) > 0)
        emit statusMsg(tr("%1: 发送数据成功").arg(peerhostname));
    //dataSocket->flush();
}
...
void tabhost::readDataIn()
{
    ...
    {
        emit statusMsg(tr("%1: 手动路由表发送成功！").arg(peerhostname));
    }
    else if (receivedMsg.startsWith(rtAutoHead))
    {
        emit statusMsg(tr("%1: 自动路由模式启动成功！").arg(peerhostname));
    }
    else if (receivedMsg.startsWith(rtManuHead))
    {
        emit statusMsg(tr("%1: 手动路由模式启动成功！").arg(peerhostname));
    }
}
